...
| Z8 | **Sign-out link** — with a live portal session, use the link in the configuration UI, then open the portal again | The portal asks again, and the configured mode applies. **Unverified:** the ESTS logout clears the ESTS session for certain, but whether it also tears down a portal's own application session and MSAL cache is Microsoft behaviour we have not measured. If it does not, the portal keeps serving its cached token until it expires and the button helps less than it promises (F8) |
